...
const main = {
	...
	buildAndRun() {
		runCommand(function buildAndRun() {
			ensureCompilerIsInstalled(function () {
				const sourceFile = getScriptFile();
...
function getConfigFile() {
	...
	if (workspaceFolder) {
		const configFile = workspaceFolder.uri.path + '/.cpp-pp.json';
...
function runCommand(callback) {
	if (useConfigFile()) {
		...
		if (configFile) {
			runFromConfigFileByKey(configFile, callback.name);
		} else {
			callback();
		}
	} else {
...
function runFromConfigFileByKey(configFile, key) {
	const data = file.readFileSync(configFile);
